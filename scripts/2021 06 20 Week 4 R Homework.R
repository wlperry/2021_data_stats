...
# the older stuff ends at line 194 roughly 
...
# QUESTION ---
# what command do you use to rename the variables ? 
...
# Question -----
# How do you make ID numeric?
# make id numeric
summary.df <- summary.df %>% 
  RRRRR(id = RRRRRR(id))
#QUESTION-----
# how do you make seed type and ID factors?
# now lets make each id a factor and seed_type
summary.df <- summary.df %>% 
  mutate(id = RRRRRR(id),
         seed_type = RRRR(seed_type))
...
#QUESTION------
# How do you filter out empty rows? 
summary.df <- summary.df %>% 
  RRRRR(RRRRR(id))
# remember is.na gives you the na values in a variable
# remeber ! gives you the opposite
...
# Question -----
# How do you rename id_from_main_protocol to id?
full.df <- full.df %>% 
  RRRRR(RRRR = RRRRR)
...
# QUESTION ----
# how do you do summaries by groups? what is the verb
# how would you get the Standard Deviation?
std_dev.df <- full.df %>% 
  RRRRR(id, type) %>% 
  summarise(
    stddev_length_mm = RRRRR(length_mm, na.rm=TRUE),
    stddev_width_mm = RRRRRR(width_mm, na.rm=TRUE)
    )

...
# Cleaning data ----
# so lets clean the data and make it a bit more usable
# note in the full_marvin 
...
# Later down the list is summarizing the data
# Data Summary -----
# we need to:
#       get mean and standard deviations by plant and counts
#       need to merge with main dataframe


# SUMMARY DATAFRAME --- 
# change data in the dataframe
# note in both the ID is a character variable and need to be numeric
...
# so how or waht command would you use to rename the variables
summary_marvin.df <- summary_marvin.df %>% 
  rename(
    species          = main_seed,
    seed_number      = main_seeds,
...
# Filter out Rows ----
# need to get ride of ID variables that are not there
summary_marvin.df <- summary_marvin.df %>% 
  filter(!is.na(id))
...
full_marvin.df <- full_marvin.df %>% dplyr::rename(id = id_from_main_protocol)
...
full_marvin.df %>% group_by(seed_type) %>% skim()
